...
export default function ChatSection({ messages, setMessages }: ChatSectionProps) {
  ...
  const parseErrorResponse = async (res: Response): Promise<string> => {
    try {
      ...
      return `HTTP ${res.status}: ${res.statusText || 'Error inesperado'}`;
    }
  };
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'agent',
          content: data?.respuesta || 'Sin respuesta del sistema.',
        },
      ]);
    } catch (error) {
      const message =
      ...
      <div className="p-4 md:p-6 bg-[#0c0c0e]/80 backdrop-blur-xl border-t border-zinc-800/50">
        ...
        >
          <input
            ...
            placeholder="Ej: ¿Cuáles son las políticas KYC o límites transaccionales?"
        ...
        <p className="text-center text-[10px] text-zinc-600 mt-3 font-medium uppercase tracking-widest">
          NovaBank AI puede cometer errores. Verifica con el área de compliance.
        </p>
      </div>
    </div>
  );
